...
static const CGFloat kloadingIndividualAnimationTiming = 0.4;
static const CGFloat kbarDarkAlpha = 0.4;
static const CGFloat kloadingTimingOffset = 0.05;
...
- (IBAction)didRotateDevice:(id)sender{
...
- (void)updateBarItemsWithProgress:(CGFloat)progress
{
    for (BarItem *barItem in self.barItems) {
        ...
        else if (progress == 1 || progress >= 1 - endPadding) {
            barItem.transform = CGAffineTransformIdentity;
...
- (void)beginLoading{
    ...
    CGFloat offsetY = -self.dropHeight - self.originalTopContentInset;
    
    Weakself(ws);
    [UIView animateWithDuration:0.3 animations:^{
        
        [[ws scrollView] setContentOffset:CGPointMake(0, offsetY)];
    } completion:^(BOOL finished) {
        
        [ws triggerLoadingAnimation];
    }];
}
