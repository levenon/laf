...
import com.fileserver.dao.IBaseDao;
...
public class BaseDao extends HibernateDaoSupport implements IBaseDao {
	 ...
	 * 根据给出例子查找对象－分页
	 */
	public List<?> queryForListByPage(Object transientObject, int firstRow,
			int rowSize) {
		return getHibernate().findByExample(transientObject, firstRow,
				rowSize);
	}
	...
	public List<?> findByProperty(String tableModel, String propertyName,
		...
		String queryString = "from " + tableModel
				+ " as model where model.isDelete = " + DefaultCode.Code_False
				+ " and model." + propertyName + "= ?" + order;
	...
	public void delAll(String[] ids, String str) {
		...
		Session session = getSession();
		String hqls = "update " + str + " as model " +
				"set model.isDelete = " + DefaultCode.Code_True +
				" , model.deleteTime = now() where model.id = " + hql;
